...
import api from "../stores/api";
    ...
    const likeComment = () => {
        api.post('liked_comments', {
            "comment": "/api/comments/"+current.comment.id,
            "user": current.utilisateur['@id']
        })
        .then((response) => {
            commentStore.fetchCommentsLike();
            console.log(response);
        })
    }

    const dislikeComment = () => {
        api.delete('liked_comments/'+current.comment.likeId)
        .then((response) => {
            commentStore.fetchCommentsLike();
            console.log(response);
        })
...
<template>
    <article class=" mr-12 mb-12 ml-12">
        ...
        <div class=" flex flex-row justify-between">
            ...
            <div v-if="current.utilisateur" class="w-20 flex flex-row justify-between text-vert ">
